...
namespace Atmos
{
    ...
    void StatusScreen::Page::Select(EntityReference selected)
    {
        root->Show();
        OnSelected();
        Update(selected);
    ...
    void StatusScreen::Stats::Update(EntityReference selected)
    {
        CombatComponentReference use;
        if (selected)
        {
    ...
    void StatusScreen::Stats::Entry::Update(CombatComponentReference selected)
    {
        if (!selected)
        {
    ...
    void StatusScreen::Inventory::Update(EntityReference selected)
    {
    ...
    void StatusScreen::Inventory::CharacterSection::Update(EntityReference selected)
    {
        if (selected)
    ...
    void StatusScreen::Inventory::ReserveSection::Update(EntityReference selected)
    {
        if (selected)
    ...
    void StatusScreen::Spells::Update(EntityReference selected)
    {
    ...
    void StatusScreen::Spells::OnSpellSelected(SpellReference spell)
    {
        if (!IsSelected())
            return;
    }

    StatusScreen::Member::Member(Agui::Image &frame) :
        frame(frame), portrait(*Agui::Image::Factory(&frame, frame.GetName() + "Portrait", Agui::RelativePosition(), 0.1f)),
        player()
    {
    ...
    const StatusScreen::PartySizeT StatusScreen::rowSize = 8;
    ...
    void StatusScreen::OnActionPressed(const Input::Action &args)
    {
        ...
        const PartySizeT halfPartySize = 8;
        switch (args.id)
        {
    ...
    void StatusScreen::Activate()
    {
        ...
        for (auto memberLoop = members.begin(); memberLoop != members.end(); ++memberLoop)
        {
            ...
            {
                memberLoop->portrait.GetSprite()->SetResource(GetCurrentEntities()->FindComponent<Ent::SenseComponent>(memberLoop->player)->sprites.begin()->Get().GetMaterial()->GetXVisual()->GetFileName().c_str());
