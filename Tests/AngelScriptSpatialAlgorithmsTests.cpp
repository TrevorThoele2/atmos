...
SCENARIO_METHOD(AngelScriptSpatialAlgorithmsTestsFixture, "running spatial algorithm AngelScript scripts", "[script][angelscript]")
{
    ...
    GIVEN("script that returns Clamp from AxisAlignedBox2D and AxisAlignedBox2D")
    {
        auto center0 = dataGeneration.RandomStack<
            Spatial::Point2D, Spatial::Point2D::Value, Spatial::Point2D::Value>();
        auto size0 = dataGeneration.RandomStack<
        ...
        CompileAndCreateScript(
            ...
            "    Atmos::Spatial::AxisAlignedBox2D box1(center1, size1);\n" \
            "\n" \
            "    const auto clamped = Atmos::Spatial::Clamp(box0, box1);\n" \
            "    return Atmos::ToString(clamped.Left())\n" \
            "        + \" \" + Atmos::ToString(clamped.Top())\n" \
            "        + \" \" + Atmos::ToString(clamped.Right())\n" \
            "        + \" \" + Atmos::ToString(clamped.Bottom());" \
            "}",
            {
                center0.x,
                center0.y,
        ...
        WHEN("working reliquary")
        {
            ...
            THEN("has correct properties")
            {
                ...
                const auto box0 = Spatial::AxisAlignedBox2D(center0, size0);
                const auto box1 = Spatial::AxisAlignedBox2D(center1, size1);
                const auto clamped = Clamp(box0, box1);

                const auto result = std::get<String>(std::get<Variant>(finishes[0].result));
                const auto expectedResult = ToString(clamped.Left())
                    + " " + ToString(clamped.Top())
                    + " " + ToString(clamped.Right())
                    + " " + ToString(clamped.Bottom());
                REQUIRE(result == expectedResult);
            }
        }
    }

    GIVEN("script that returns Clamp from AxisAlignedBox3D and AxisAlignedBox3D")
    {
        auto center0 = dataGeneration.RandomStack<
            Spatial::Point3D, Spatial::Point3D::Value, Spatial::Point3D::Value, Spatial::Point3D::Value>();
        auto size0 = dataGeneration.RandomStack<
        ...
        CompileAndCreateScript(
            ...
            "    return Atmos::ToString(clamped.Left())\n" \
            "        + \" \" + Atmos::ToString(clamped.Top())\n" \
            "        + \" \" + Atmos::ToString(clamped.FarZ())\n" \
            "        + \" \" + Atmos::ToString(clamped.Right())\n" \
            "        + \" \" + Atmos::ToString(clamped.Bottom())\n" \
            "        + \" \" + Atmos::ToString(clamped.NearZ());\n" \
            "}",
            {
                center0.x,
                center0.y,
        ...
        WHEN("working reliquary")
        {
            ...
            THEN("has correct properties")
            {
                ...
                const auto expectedResult = ToString(clamped.Left())
                    + " " + ToString(clamped.Top())
                    + " " + ToString(clamped.FarZ())
                    + " " + ToString(clamped.Right())
                    + " " + ToString(clamped.Bottom())
                    + " " + ToString(clamped.NearZ());
                REQUIRE(result == expectedResult);
            }
        }
    }
